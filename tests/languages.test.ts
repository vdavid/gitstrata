...
describe('isTestFile', () => {
    ...
    it('detects C++ test file patterns', () => {
        expect(isTestFile('test_math.cpp', ['test_*.cpp', '*_test.cpp', '*_test.cc', '*_test.cxx'])).toBe(true)
        expect(isTestFile('math_test.cc', ['test_*.cpp', '*_test.cpp', '*_test.cc', '*_test.cxx'])).toBe(true)
    })
...
describe('countZigInlineTestLines', () => {
    ...
    it('handles test block with no description', () => {
        const content = ['pub fn foo() void {}', '', 'test {', '    try std.testing.expect(true);', '}'].join('\n')
        expect(countZigInlineTestLines(content)).toBe(3)
    })
})

describe('noTestSplit languages', () => {
    const noTestSplitIds = ['html', 'css', 'sql', 'shell', 'svelte', 'vue', 'astro', 'docs', 'config']
