...
${statBlock}`
    ...
    const cloneSubPhaseComplete = $derived(
        phase === 'cloning' && cloneTotal > 0 && cloneLoaded >= cloneTotal && clonePhase === completedClonePhase,
    )
    ...
    const activeIndex = $derived(activePhaseId ? phases.findIndex((p) => p.id === activePhaseId) : -1)
    ...
    const getPhaseProgress = (
        ...
        if (cloneSubPhaseComplete && phaseDef.id !== 'analyze') return { show: false, percent: 0, color: '' }
    ...
    const totalElapsed = $derived(phase === 'processing' ? cloneElapsedMs + processElapsedMs : cloneElapsedMs)
        ...
        {#each phases as phaseDef, i (phaseDef.id)}
            ...
            <li class="phase-row" aria-current={state === 'active' ? 'step' : undefined}>
                ...
                <div class="phase-indicator">
                    ...
                    {#if !isLast}
                        <span class="phase-connector" class:phase-connector--done={state === 'done'} aria-hidden="true"
                        ></span>
                    {/if}
                </div>
